...
# the first wave householdID's.

# ids is a vector of strings, all of length 9

CHARLS_hhids <- function(ids){
  # Length of the vector ids
  n <- length(ids)
  # Build a matrix where each row corresponds to an element of ids
  ids_s <- matrix(unlist(strsplit(ids, "")), byrow = TRUE, nrow=n, ncol = 9)
  # Add a column with zeroes in the 10th position.
  ids_s2 <- cbind(ids_s[,1:9], rep(0,n))
