...
label(CHARLS_w1)=lapply(names(CHARLS_w1),function(x) label(CHARLS_w1[,x]) = label(CHARLS_w1_2[x]))
...
label(CHARLS_w2)=lapply(names(CHARLS_w2),function(x) label(CHARLS_w2[,x]) = label(CHARLS_w2_2[x]))
...
```{r summ}
CHARLS_ds_w1$retired <- labelled(CHARLS_ds_w1$retired, labels = cat_label)
label(CHARLS_ds_w1$retired) <- ds_label
CHARLS_ds_w2$retired <- labelled(CHARLS_ds_w2$retired, labels = cat_label)
label(CHARLS_ds_w2$retired) <- ds_label
CHARLS_ds_w1$age_retired <- labelled(CHARLS_ds_w1$age_retired, labels = cont_label)
label(CHARLS_ds_w1$age_retired) <- ds_label_age
CHARLS_ds_w2$age_retired <- labelled(CHARLS_ds_w2$age_retired, labels = cont_label)
label(CHARLS_ds_w2$age_retired) <- ds_label_age


CHARLS_m_ds_w1 <- CHARLS_ds_w1
CHARLS_m_ds_w1$retired <- car::recode(CHARLS_m_ds_w1$retired, "miss_values_vector=NA")
CHARLS_m_ds_w2 <- CHARLS_ds_w2
CHARLS_m_ds_w2$retired <- car::recode(CHARLS_m_ds_w2$retired, "miss_values_vector=NA")


tt <-    frq(CHARLS_ds_w1$retired)[[1]][,c(1:2,4)]
tt[4] <- frq(CHARLS_ds_w2$retired)[[1]][,c(4)]
tt[13,] <- c("n", "sample size", 
             length(CHARLS_ds_w1$retired), length(CHARLS_ds_w2$retired)
             )
names(tt)<-c("val", "label", "w1", "w2")
kable(tt)

dbb <- merge(CHARLS_m_ds_w1[,c("id","retired")], CHARLS_m_ds_w2[,c("id","retired")], by="id", suffixes = c(".w1",".w2"), all=T)
...
for(index in seq_along(l.CHARLS)){
  retired <- l.CHARLS[[index]]
  save(retired, file = paste0(datafolder,names(l.CHARLS)[index],"/retired.RData"))
}
