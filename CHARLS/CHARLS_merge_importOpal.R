...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/CHARLS")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_CHARLS.Rdata")
...
lhd <- list("charls_w1_hd", "charls_w2_hd")
...
for (i in 1:length(lhd)) {
  ...
  # Add athlos_id
  ds <- left_join(ds, ids_athlos_ds_CHARLS, by = 'id')
  ...
  opal.symbol_import(o, lhd[[i]], project='CHARLS_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_CHARLS","hs") )) 
}
