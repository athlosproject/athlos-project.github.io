...
cpd_age <- ALSA_w1$DIAGYR7[which(ALSA_ds_w1$h_cpd == 1)]-ALSA_w1$ybirth[which(ALSA_ds_w1$h_cpd == 1)]
...
ALSA_ds$h_cpd_age[which(ALSA_ds$id %in% ALSA_ds_w6$id)] <- ALSA_ds_w6$h_cpd_age
...
* The harmonised variable contains information about "chronic bronchitis" and "emphysema".
* The minimum age of diagnostic is zero in waves 1 and 6 because there are two individuals of wave 1 (of which one follows in wave 6) which answer year of diagnostic equal to year of birth.

```{r echo=F}
kable(ALSA_w1[which(ALSA_ds_w1$h_cpd_age==0),], align = 'c')
```
