...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/ALSA")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_ALSA.Rdata")
...
lf   <- list("./RData/w1", "./RData/w2", "./RData/w3", "./RData/w4", "./RData/w5", "./RData/w6", 
             "./RData/w7", "./RData/w8", "./RData/w9", "./RData/w10", "./RData/w11", "./RData/w12", "./RData/w13")

# lhd is a list of harmonised table names.
lhd <- list("alsa_w1_hd", "alsa_w2_hd", "alsa_w3_hd", "alsa_w4_hd", "alsa_w5_hd", "alsa_w6_hd", 
            "alsa_w7_hd", "alsa_w8_hd", "alsa_w9_hd", "alsa_w10_hd", "alsa_w11_hd", "alsa_w12_hd", "alsa_w13_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_ALSA, by = 'id')

    
  #variable country
  ds$country <- 81  # Australia

  #variable cohort
  ds$cohort <- "South Australia"  
  ...
  opal.symbol_import(o, lhd[[i]], project='ALSA_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var","text.var", "i","ids_athlos_ds_ALSA","hs") )) 
}
