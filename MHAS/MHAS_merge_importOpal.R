...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/MHAS")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_MHAS.Rdata")
...
lhd <- list("mhas_w1_hd", "mhas_w2_hd", "mhas_w3_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_MHAS, by = 'id')
  ...
  # Remove 4 individuals that only appear at wave 3 and they are dead
  if (i==3) {ds <- ds %>% filter(!id %in% c("1054920.0", "524220.0", "729811.0", "931111.0"))}

  # "ds" tibble is imported to Opal
  opal.assign.data(  o, lhd[[i]], ds)
  opal.symbol_import(o, lhd[[i]], project='MHAS_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_MHAS","hs") )) 
}
