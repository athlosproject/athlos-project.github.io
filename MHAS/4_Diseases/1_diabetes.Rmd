...
| **Missings** | `8 = Doesn't answer; 9 = Doesn't know; NA` | `995 = Does not answer; 998 = Do not know; 999 = Missing` |  |  |
| **Description** |  |  |  |  |
...
MHAS_ds_w2$h_diabetes[((!is.na(MHAS_w2$c6)) & MHAS_w2$c6 == 2) & (MHAS_w2$h_diabetes != 1)] <- 0
MHAS_ds_w2$h_diabetes[((!is.na(MHAS_w2$c6)) & MHAS_w2$c6 == 1) | ((!is.na(MHAS_w2$h_diabetes)) & (!is.na(MHAS_w2$c6)) & MHAS_w2$h_diabetes == 1)] <- 1
MHAS_ds_w2$h_diabetes[((!is.na(MHAS_w2$c6)) & MHAS_w2$c6 == 8) & (MHAS_w2$h_diabetes != 1)] <- 995
MHAS_ds_w2$h_diabetes[((!is.na(MHAS_w2$c6)) & MHAS_w2$c6 == 9) & (MHAS_w2$h_diabetes != 1)] <- 998

# Continuous variable harmonization
diabetes_age <- MHAS_w2$age[which(MHAS_ds_w2$h_diabetes == 1 & MHAS_w2$h_diabetes==0 & !MHAS_w2$age %in% miss_values_vector)]-(MHAS_w2$yintw_w2[which(MHAS_ds_w2$h_diabetes == 1 & MHAS_w2$h_diabetes==0 & !MHAS_w2$age %in% miss_values_vector)]-MHAS_w2$yintw_w1[which(MHAS_ds_w2$h_diabetes == 1 & MHAS_w2$h_diabetes==0 & !MHAS_w2$age %in% miss_values_vector)])/2
MHAS_ds_w2$h_diabetes_age <- rep(999,length(MHAS_w2$id))
MHAS_ds_w2$h_diabetes_age[MHAS_ds_w2$h_diabetes == 0] <- 996
...
pander(Continuous_summary(vbl, missing_values = miss_values_vector)$summary, caption = "Summary")
ggplot(MHAS_ds_w2, aes(h_diabetes_age)) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label_age) + ylab("Frequency")+xlim(35,100)
...
| **Categories** | `1 = Yes; 2 = No` |`continuous`|`continuous`|`continuous`|
| **Missings** | `8 = Doesn't answer; 9 = Doesn't know; NA` | `999 = Missing` |  |  |
| **Description** |  |  |  |  |
...
MHAS_ds_w3$h_diabetes[((!is.na(MHAS_w3$c6_12)) & MHAS_w3$c6_12 == 2) & (MHAS_w3$h_diabetes != 1)] <- 0
MHAS_ds_w3$h_diabetes[((!is.na(MHAS_w3$c6_12)) & MHAS_w3$c6_12 == 1) | ((!is.na(MHAS_w3$h_diabetes)) & (!is.na(MHAS_w3$c6_12)) & MHAS_w3$h_diabetes == 1)] <- 1
MHAS_ds_w3$h_diabetes[((!is.na(MHAS_w3$c6_12)) & MHAS_w3$c6_12 == 8) & (MHAS_w3$h_diabetes != 1)] <- 995
MHAS_ds_w3$h_diabetes[((!is.na(MHAS_w3$c6_12)) & MHAS_w3$c6_12 == 9) & (MHAS_w3$h_diabetes != 1)] <- 998

# Continuous variable harmonization
MHAS_ds_w3$h_diabetes_age <- rep(999,length(MHAS_w3$id))
MHAS_ds_w3$h_diabetes_age[MHAS_ds_w3$h_diabetes == 0] <- 996
...
MHAS_ds_w3$h_diabetes_age[which(MHAS_ds_w3$h_diabetes == 1 & MHAS_w3$h_diabetes_w2==0)] <- (MHAS_w3$yintw_w3[which(MHAS_ds_w3$h_diabetes == 1 & MHAS_w3$h_diabetes_w2==0)]+MHAS_w3$yintw_w2[which(MHAS_ds_w3$h_diabetes == 1 & MHAS_w3$h_diabetes_w2==0)])/2-MHAS_w3$ybirth[which(MHAS_ds_w3$h_diabetes == 1 & MHAS_w3$h_diabetes_w2==0)]
...
kable(Continuous_summary(vbl, missing_values = miss_values_vector)[2], caption = "Values")
pander(Continuous_summary(vbl, missing_values = miss_values_vector)$summary, caption = "Summary")
ggplot(MHAS_ds_w3, aes(h_diabetes_age)) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label_age) + ylab("Frequency")+xlim(25,90)
...
```{r summ}
MHAS_ds_w1$h_diabetes <- labelled(MHAS_ds_w1$h_diabetes, labels = cat_label)
label(MHAS_ds_w1$h_diabetes) <- ds_label
MHAS_ds_w2$h_diabetes <- labelled(MHAS_ds_w2$h_diabetes, labels = cat_label)
label(MHAS_ds_w2$h_diabetes) <- ds_label
MHAS_ds_w2$h_diabetes_age <- labelled(MHAS_ds_w2$h_diabetes_age, labels = cont_label)
label(MHAS_ds_w2$h_diabetes_age) <- ds_label_age
MHAS_ds_w3$h_diabetes <- labelled(MHAS_ds_w3$h_diabetes, labels = cat_label)
label(MHAS_ds_w3$h_diabetes) <- ds_label
MHAS_ds_w3$h_diabetes_age <- labelled(MHAS_ds_w3$h_diabetes_age, labels = cont_label)
label(MHAS_ds_w3$h_diabetes_age) <- ds_label_age


MHAS_m_ds_w1 <- MHAS_ds_w1
MHAS_m_ds_w1$h_diabetes <- car::recode(MHAS_m_ds_w1$h_diabetes, "miss_values_vector=NA")
MHAS_m_ds_w2 <- MHAS_ds_w2
MHAS_m_ds_w2$h_diabetes <- car::recode(MHAS_m_ds_w2$h_diabetes, "miss_values_vector=NA")
MHAS_m_ds_w3 <- MHAS_ds_w3
MHAS_m_ds_w3$h_diabetes <- car::recode(MHAS_m_ds_w3$h_diabetes, "miss_values_vector=NA")


tt <-    frq(MHAS_ds_w1$h_diabetes)[[1]][,c(1:2,4)]
tt[4] <- frq(MHAS_ds_w2$h_diabetes)[[1]][,c(4)]
tt[5] <- frq(MHAS_ds_w3$h_diabetes)[[1]][,c(4)]
tt[13,] <- c("n", "sample size", 
             length(MHAS_ds_w1$h_diabetes), length(MHAS_ds_w2$h_diabetes), length(MHAS_ds_w3$h_diabetes)
             )
names(tt)<-c("val", "label", "w1", "w2", "w3")
kable(tt)

dbb <- merge(MHAS_m_ds_w1[,c("id","h_diabetes")], MHAS_m_ds_w2[,c("id","h_diabetes")], by="id", suffixes = c(".w1",".w2"), all=T)
dbb <- merge(dbb, MHAS_m_ds_w3[,c("id","h_diabetes")], by="id", all=T)
...
for(index in seq_along(l.MHAS)){
  h_diabetes <- l.MHAS[[index]]
  save(h_diabetes, file = paste0(datafolder,names(l.MHAS)[index],"/h_diabetes.RData"))
}
