...
ENRICA_ds_w1$employed <- car::recode(ENRICA_w1$w1labourforce,"c(2,3,4,6) = 0; NA = 999")
...
vbl <- ENRICA_ds_w1$employed
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[3], caption = ds_label)
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[1], caption = "Category")
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[2], caption = "Type of missing")
ggplot(ENRICA_ds_w1, aes(x=factor(employed))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label) + ylab("Frequency")
...
AfterH <- table(ENRICA_ds_w1$employed)
X <- table(ENRICA_w1$w1labourforce, useNA = "ifany")
BeforeH <- c(sum(X[c("2","3","4","6")],na.rm = T),sum(X[c("1")]),X[length(X)])
C <- rbind(BeforeH,AfterH)
colnames(C) <- c("2,3,4,6->0","1->1","NA->999")
kable(C)
...
ENRICA_ds_w2$employed <- car::recode(ENRICA_w2$w2labourforce,"c(2,3,4,6) = 0; NA = 999")
...
vbl <- ENRICA_ds_w2$employed
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[3], caption = ds_label)
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[1], caption = "Category")
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[2], caption = "Type of missing")
ggplot(ENRICA_ds_w2, aes(x=factor(employed))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label) + ylab("Frequency")
...
AfterH <- table(ENRICA_ds_w2$employed)
...
ENRICA_ds_w3$employed <- car::recode(ENRICA_w3$w3labourforce,"c(2,3,4,6) = 0; NA = 999")
...
vbl <- ENRICA_ds_w3$employed
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[3], caption = ds_label)
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[1], caption = "Category")
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[2], caption = "Type of missing")
ggplot(ENRICA_ds_w3, aes(x=factor(employed))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label) + ylab("Frequency")
...
AfterH <- table(ENRICA_ds_w3$employed)
...
```{r summ}
ENRICA_ds_w1$employed <- labelled(ENRICA_ds_w1$employed, labels = cat_label)
label(ENRICA_ds_w1$employed) <- ds_label
ENRICA_ds_w2$employed <- labelled(ENRICA_ds_w2$employed, labels = cat_label)
label(ENRICA_ds_w2$employed) <- ds_label
ENRICA_ds_w3$employed <- labelled(ENRICA_ds_w3$employed, labels = cat_label)
label(ENRICA_ds_w3$employed) <- ds_label

ENRICA_m_ds_w1 <- ENRICA_ds_w1
ENRICA_m_ds_w1$employed <- car::recode(ENRICA_m_ds_w1$employed, "miss_values_vector=NA")
ENRICA_m_ds_w2 <- ENRICA_ds_w2
ENRICA_m_ds_w2$employed <- car::recode(ENRICA_m_ds_w2$employed, "miss_values_vector=NA")
ENRICA_m_ds_w3 <- ENRICA_ds_w3
ENRICA_m_ds_w3$employed <- car::recode(ENRICA_m_ds_w3$employed, "miss_values_vector=NA")


tt <-    frq(ENRICA_ds_w1$employed)[[1]][,c(1:2,4)]
tt[4] <- frq(ENRICA_ds_w2$employed)[[1]][,c(4)]
tt[5] <- frq(ENRICA_ds_w3$employed)[[1]][,c(4)]
tt[13,] <- c("n", "sample size", 
             length(ENRICA_ds_w1$employed), length(ENRICA_ds_w2$employed), length(ENRICA_ds_w3$employed)
             )
names(tt)<-c("val", "label", "w1", "w2", "w3")
kable(tt)

dbb <- merge(ENRICA_m_ds_w1[,c("id","employed")], ENRICA_m_ds_w2[,c("id","employed")], by="id", suffixes = c(".w1",".w2"), all=T)
dbb <- merge(dbb, ENRICA_m_ds_w3[,c("id","employed")], by="id", all=T)
...
for(index in seq_along(l.ENRICA)){
  employed <- l.ENRICA[[index]]
  save(employed, file = paste0(datafolder,names(l.ENRICA)[index],"/employed.RData"))
}
