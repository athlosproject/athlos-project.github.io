...
| **Name** | `br002_` |
|-|-|
| **Label** | `smoke at the present time` |
| **Table name** | `sharew1_rel2-6-0_br` |
| **Categories:**| `1 = yes` <br/> `5 = no`|
| **Missings:**|`-1 = don't know` <br/> `NA`|
| **Description** |  |

   

```{r assign w1, echo=F}
opal.assign.table.tibble(o, 'SHARE_w1','SHARE.sharew1_rel2-6-0_br', variables=list('br002_'), missings = TRUE)
...
```{r assign w2, echo=F}
opal.assign.table.tibble(o, 'SHARE_w2','SHARE.sharew2_rel2-6-0_br', variables=list('br002_'), missings = TRUE)
...
```{r assign w4, echo=F}
opal.assign.table.tibble(o, 'SHARE_w4','SHARE.sharew4_rel1-1-1_br', variables=list('br002_'), missings = TRUE)
...
```{r assign w5, echo=F}
opal.assign.table.tibble(o, 'SHARE_w5','SHARE.sharew5_rel1-0-0_br', variables=list('br002_'), missings = TRUE)
...
dbb <- get(paste0("SHARE_m_ds_",names(l.hds)[1]))[,c("id","current_smoking")]
for(ind in 2:length(l.hds)){
  dbb <- merge(dbb, get(paste0("SHARE_m_ds_",names(l.hds)[ind]))[,c("id","current_smoking")], by = "id", suffixes = c("", paste0(".",names(l.hds)[ind])), all = T)
}
