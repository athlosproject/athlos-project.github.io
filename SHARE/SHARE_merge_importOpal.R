...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/SHARE")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_SHARE.Rdata")
...
lf   <- list("./RData/w1", "./RData/w2", "./RData/w3", "./RData/w4", "./RData/w5")

# lhd is a list of harmonised table names.
lhd <- list("share_w1_hd", "share_w2_hd", "share_w3_hd", "share_w4_hd", "share_w5_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_SHARE, by = 'id')
  ...
  opal.symbol_import(o, lhd[[i]], project='SHARE_HD')

  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_SHARE","hs") )) 
}
