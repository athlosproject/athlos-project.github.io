...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/JSTAR")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_JSTAR.Rdata")
...
lf   <- list("./RData/aksst_w1", "./RData/aksst_w2", "./RData/aksst_w3",
             "./RData/tn_w1", "./RData/tn_w2",
             "./RData/cth_w1")

# lhd is a list of harmonised table names.
lhd <- list("jstar_aksst_w1_hd"  , "jstar_aksst_w2_hd", "jstar_aksst_w3_hd",
            "jstar_tn_w1_hd", "jstar_tn_w2_hd",
            "jstar_cth_w1_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_JSTAR, by = 'id')
  ...
  # variable cohort
  if (i %in% c(1,2,3)) ds$cohort <- "aksst - 5cities"   
  if (i %in% c(4,5))   ds$cohort <- "tn - 2 cities"
  if (i %in% c(6))     ds$cohort <- "cth - 3 cities"
  ...
  opal.symbol_import(o, lhd[[i]], project='JSTAR_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_JSTAR","hs") )) 
}
