...
arthritis_age <- car::recode(TILDA_w1$ph305[TILDA_ds_w1$h_joint_disorders==1],"49=40;59=50;69=60;-98=998")
...
kable(Continuous_summary(vbl, missing_values = miss_values_vector)[2], caption = "Values")
pander(Continuous_summary(vbl, missing_values = miss_values_vector)$summary, caption = "Summary")
ggplot(TILDA_ds_w2, aes(h_joint_disorders_age)) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label_age) + ylab("Frequency")+xlim(35,85)
...
```{r summ}
TILDA_ds_w1$h_joint_disorders <- labelled(TILDA_ds_w1$h_joint_disorders, labels = cat_label)
label(TILDA_ds_w1$h_joint_disorders) <- ds_label
TILDA_ds_w1$h_joint_disorders_age <- labelled(TILDA_ds_w1$h_joint_disorders_age, labels = cont_label)
label(TILDA_ds_w1$h_joint_disorders_age) <- ds_label_age
TILDA_ds_w2$h_joint_disorders <- labelled(TILDA_ds_w2$h_joint_disorders, labels = cat_label)
label(TILDA_ds_w2$h_joint_disorders) <- ds_label
TILDA_ds_w2$h_joint_disorders_age <- labelled(TILDA_ds_w2$h_joint_disorders_age, labels = cont_label)
label(TILDA_ds_w2$h_joint_disorders_age) <- ds_label_age

TILDA_m_ds_w1 <- TILDA_ds_w1
TILDA_m_ds_w1$h_joint_disorders <- car::recode(TILDA_m_ds_w1$h_joint_disorders, "miss_values_vector=NA")
TILDA_m_ds_w2 <- TILDA_ds_w1
TILDA_m_ds_w2$h_joint_disorders <- car::recode(TILDA_m_ds_w2$h_joint_disorders, "miss_values_vector=NA")

tt <- frq(TILDA_ds_w1$h_joint_disorders)[[1]][,c(1:2,4)]
tt[4] <- frq(TILDA_ds_w2$h_joint_disorders)[[1]][,c(4)]
tt[13,] <- c("n", "sample size", 
             length(TILDA_ds_w1$h_joint_disorders), length(TILDA_ds_w2$h_joint_disorders)
             )
names(tt)<-c("val", "label", "w1", "w2")
kable(tt)

dbb <- merge(TILDA_m_ds_w1[,c("id","h_joint_disorders")], TILDA_m_ds_w2[,c("id","h_joint_disorders")], by="id", suffixes = c(".w1",".w2"), all=T)
...
for(index in seq_along(l.TILDA)){
  h_joint_disorders <- l.TILDA[[index]]
  save(h_joint_disorders, file = paste0(datafolder,names(l.TILDA)[index],"/h_joint_disorders.RData"))
}
