...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/TILDA")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_TILDA.Rdata")
...
lhd <- list("tilda_w1_hd", "tilda_w2_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_TILDA, by = 'id')

    
  #variable country
  ds$country <- 62 # Ireland
  ...
  opal.symbol_import(o, lhd[[i]], project='TILDA_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_TILDA","hs") )) 
}
