...
| **Name** | `fl002_5`|`DISimpairments`|
|-|-|-|
| **Label** | `Because of a physical or mental health problem, do you have difficulty Getting in or out of bed` |`Number of physical limitations (from fl001)`\*|
...
opal.assign.table.tibble(o, 'TILDA_w1','TILDA.TILDA-AMF-v16', variables=list('fl002_5','DISimpairments'), missings = TRUE)
...
vbl <- TILDA_w1$fl002_5
vbl_miss <- c(-1)
kable(Categorical_summary(vbl, missing_values = vbl_miss)[3], caption = attributes(vbl)$`spss::shortName`)
kable(Categorical_summary(vbl, missing_values = NA)[1], caption = "Category")
ggplot(TILDA_w1, aes(x=factor(vbl))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab("difficulty Getting in or out of bed") + ylab("Frequency")

kable(table(TILDA_w1$fl002_5,TILDA_w1$DISimpairments, useNA = "ifany"), caption = 'Cross data of fl002_5 with DISimpairments')

```

#### Harmonisation algorithm

To compute the harmonize variable from the study-specific variable `fl002_5` it has to be recoded as follows:
