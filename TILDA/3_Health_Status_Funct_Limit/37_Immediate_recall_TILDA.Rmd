...
|-|-|-|-|
| **Label** | `Interviewer will read the list instead of the computer` | `Now please tell me all the words you can recall. IWER ENTER THE NUMBER OF WORDS`| `Now please tell me all the words you can recall. Number of words the respondent correctly recalls (1st read by interviewer)`|
...
| **Missings** |  | `-99 = Refused; -98 = Don't know`|`-99 = Refused; -98 = Don't know`|
| **Description** |  |  |  |
...
opal.assign.table.tibble(o, 'TILDA_w1_cog_imm','TILDA.TILDA-AMF-v16', variables=list('ph116','ph117','ph119'), missings = TRUE)
TILDA_w1_cog_imm <- opal.execute(o,'TILDA_w1_cog_imm')
```

```{r local w1, echo=FALSE}
vari <- TILDA_w1_cog_imm$ph116
kable(Categorical_summary(vari, missing_values = NA)[2], caption = "ph116")
kable(Categorical_summary(vari, missing_values = NA)[1], caption = "Category")
ggplot(TILDA_w1_cog_imm, aes(x=factor(vari))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(label(vari)) + ylab("Frequency")

vari <- TILDA_w1_cog_imm$ph117
vbl_miss <- c(-99,-98)
kable(Continuous_summary(var = vari, missing_values = vbl_miss)[3], caption = "ph117")
...
vari <- TILDA_w1_cog_imm$ph119
vbl_miss <- c(-99,-98)
kable(Continuous_summary(var = vari, missing_values = vbl_miss)[3], caption = "ph119")
kable(Continuous_summary(vari, missing_values = vbl_miss)[2], caption = "Values")
pander(Continuous_summary(vari, missing_values = vbl_miss)$summary, caption = "Summary")
ggplot(TILDA_w1_cog_imm, aes(x=factor(vari))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(label(vari)) + ylab("Frequency")
```
...
TILDA_w1_cog_imm <- TILDA_w1_cog_imm %>% 
  mutate(cog_imm_c = case_when(ph116 == 1 ~ as.integer(ph117),ph116 == 2 ~ as.integer(ph119)))
TILDA_ds_w1$cog_imm_c <- car::recode(TILDA_w1_cog_imm$cog_imm_c, "-98 = 0; -99 = 997")

##cog_imm
Q <- quantile(car::recode(TILDA_ds_w1$cog_imm_c, "miss_values_vector=NA"), probs = 1/4, na.rm = TRUE, names = FALSE)
...
ggplot(TILDA_ds_w1, aes(x=factor(cog_imm))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label) + ylab("Frequency")
```
...
kable(Categorical_summary(vari, missing_values = NA)[2], caption = "ph116")
kable(Categorical_summary(vari, missing_values = NA)[1], caption = "Category")
ggplot(TILDA_w2_cog_imm, aes(x=factor(vari))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(label(vari)) + ylab("Frequency")
...
TILDA_w2_cog_imm <- TILDA_w2_cog_imm %>% 
  mutate(cog_imm_c = case_when(ph116 == 1 ~ as.integer(ph117),ph116 == 2 ~ as.integer(ph119)))

TILDA_ds_w2$cog_imm_c <- car::recode(TILDA_w2_cog_imm$cog_imm_c, "100 = 994; -1 = 996")
...
kable(Categorical_summary(vari, missing_values = miss_values_vector)[1], caption = "Category")
kable(Categorical_summary(vari, missing_values = miss_values_vector)[2], caption = "Type of missing")
ggplot(TILDA_ds_w2, aes(x=factor(cog_imm))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label) + ylab("Frequency")
```
...
Dichotomic <- table(TILDA_ds_w2[[3]], useNA = "ifany")
