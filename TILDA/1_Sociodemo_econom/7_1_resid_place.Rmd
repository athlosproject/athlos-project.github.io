...
| **Name** | `local3` |
|-|-|
| **Label** | `Location of household - Dublin/Urban/Rural` |
| **Table name** | `TILDA-AMF-v16` |
| **Categories** | `1 = Dublin city or county`<br/>`2 = Another town or city`<br/>`3 = A rural area`|
| **Missings** | `NA`|
| **Description** |  |

```{r assign w1, echo=F}
opal.assign.table.tibble(o, 'TILDA_w1','TILDA.TILDA-AMF-v16', variables=list('local3'), missings = TRUE)
...
TILDA_ds_w1$resid_place <- car::recode(TILDA_w1$local3, "c(1,2) = 0; 3 = 1; NA = 999")

```

#### Statistical description of the new harmonised variable
```{r descript w1, echo=F}
vbl <- TILDA_ds_w1$resid_place
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[3], caption = ds_label)
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[1], caption = "Category")
kable(Categorical_summary(vbl, missing_values = miss_values_vector)[2], caption = "Type of missing")
ggplot(TILDA_ds_w1, aes(x=factor(resid_place))) + geom_bar(stat="count", width=0.4, fill="steelblue") + xlab(ds_label) + ylab("Frequency")
```
