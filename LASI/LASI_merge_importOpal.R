...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/LASI")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_LASI, by = 'id')
  ...
  opal.symbol_import(o, lhd[[i]], project='LASI_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_LASI","hs") )) 
}
