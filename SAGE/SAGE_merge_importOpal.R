...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/SAGE")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_SAGE.Rdata")
...
lf   <- list("./RData/china_w1", "./RData/ghana_w1", "./RData/india_w1", "./RData/mexico_w1", "./RData/russia_w1", "./RData/SA_w1")

# lhd is a list of harmonised table names.
lhd <- list("sage_china_w1_hd", "sage_ghana_w1_hd", "sage_india_w1_hd", "sage_mexico_w1_hd", "sage_russia_w1_hd", "sage_sa_w1_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_SAGE, by = 'id')
  ...
  if (i %in% c(5)) {
    ds$country <- 41 # Russia
    ds$cohort <- "Russia"
  }
  if (i %in% c(6)) {
    ds$country <- 12 # South Africa
    ds$cohort <- "South Africa"
  }
  ...
  opal.symbol_import(o, lhd[[i]], project='SAGE_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_SAGE","hs") )) 
}
