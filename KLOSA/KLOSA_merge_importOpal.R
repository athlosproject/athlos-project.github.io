...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/KLOSA")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_KLOSA.Rdata")
...
lf   <- list("./RData/w1", "./RData/w2", "./RData/w3", "./RData/w4")

# lhd is a list of harmonised table names.
lhd <- list("klosa_w1_hd", "klosa_w2_hd", "klosa_w3_hd", "klosa_w4_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_KLOSA, by = 'id')

    
  #variable country
  ds$country <- 35 # South Korea

  #variable cohort
  ds$cohort <- "South Korea"  
  ...
  opal.symbol_import(o, lhd[[i]], project='KLOSA_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_KLOSA","hs") )) 
}
