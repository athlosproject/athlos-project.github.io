...
setwd("M:/WPs/WP1/Maelstrom/data_process/athlos-project.github.io/HAPIEE")
# Load some useful packages and Opal session
source("../setup.r")

# Load data base with 'athlos_id' and 'athlos_id2' for id's in current study
load("M:/WPs/WP1/Maelstrom/data_process/ATHLOS_cohort/ids2/ids_athlos_ds_HAPIEE.Rdata")
...
lf   <- list("./RData/pol_w1", "./RData/pol_w2",
             "./RData/cz_w1", "./RData/cz_w2",
             "./RData/li_w1")

# lhd is a list of harmonised table names.
lhd <- list("hapiee_pol_w1_hd"  , "hapiee_pol_w2_hd",
            "hapiee_cz_w1_hd", "hapiee_cz_w2_hd",
            "hapiee_li_w1_hd")
...
for (i in 1:length(lhd)) {
  ...
  ds <- left_join(ds, ids_athlos_ds_HAPIEE, by = 'id')
  ...
  if (i %in% c(3,4)) {
    ds$country <- 54 #Czech Republic
    ds$cohort <- "Czech Republic"
  }
  if (i %in% c(5)) {
    ds$country <- 64   #Lithuania
    ds$cohort <- "Lithuania"
  }
  ...
  opal.symbol_import(o, lhd[[i]], project='HAPIEE_HD')
  
  # Remove all objects but the listed in the vector.
  rm(list=setdiff(ls(), c("o","lf", "lhd", "integ.var", "text.var", "i","ids_athlos_ds_HAPIEE","hs") )) 
}
